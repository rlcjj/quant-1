...
class FundHolderRiskAlphaReturnHalfYear(Data):
    ...
    def __init__(self):
        ...
        Data.__init__(self)
        self.sub_data_path = r'fund_data\fund_exposure\fund_holding_risk_alpha_return_halfyear'
        self.halfyear_return_path = os.path.join(self.primary_data_path, self.sub_data_path)

    def cal_fund_holder_risk_alpha_return_halfyear(self, fund, end_date):
        ...
        fund_exposure = FundHolderExposureHalfYear().get_fund_holder_exposure_halfyear_daily(fund, beg_date, end_date)
        ...
        # 合并新数据
        ####################################################################
        out_file = os.path.join(self.halfyear_return_path,
                                'Fund_Holder_Risk_Alpha_Return_HalfYear_' + fund + "_" + end_date + '.csv')
        print(out_file)
    ...
    def cal_fund_holder_risk_alpha_return_halfyear_all(self,
                                                       beg_date="20040101",
                                                       end_date=datetime.today().strftime("%Y%m%d"),
                                                       fund_pool="股票+灵活配置60型基金"):

        """ 根据半年报持仓风格暴露进行收益拆分 所有基金"""
        ...
        for i_date in range(0, len(date_series)):
            ...
            for i_fund in range(0, len(fund_pool_list)):
                ...
                self.cal_fund_holder_risk_alpha_return_halfyear(fund_code, end_date)

    def get_fund_holder_risk_alpha_return_halfyear(self, fund, report_date):

        """ 得到半年报持仓风格暴露进行收益拆分 单个基金"""

        out_file = os.path.join(self.halfyear_return_path,
                                'Fund_Holder_Risk_Alpha_Return_HalfYear_' + fund + "_" + report_date + '.csv')
...
if __name__ == "__main__":
    ...
    self = FundHolderRiskAlphaReturnHalfYear()
    fund, beg_date, end_date = "000001.OF", "20181031", "20181231"
    self.cal_fund_holder_risk_alpha_return_halfyear_all(beg_date, end_date)

    # print(self.cal_fund_holder_risk_alpha_return_halfyear("000001.OF", "20171231"))
    # self.cal_fund_holder_risk_alpha_return_halfyear_all("20151231", "20180909")
    # print(self.get_fund_holder_risk_alpha_return_halfyear("000001.OF", "20171231"))
