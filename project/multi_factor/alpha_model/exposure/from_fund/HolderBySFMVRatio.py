...
def HolderBySFMVRatio(beg_date, end_date):

    """
    因子说明：被主动股票基金季报持有在前十大重仓的持有的金额（比例*基金规模*基金股票比例）/自由流通市值
    ...
    fund_asset = Fund().get_fund_factor('Total_Asset', None, None)
    ...
    for i_fund in range(0, len(fund_pool)):
        ...
        if fund_holding is not None:
            ...
            fund_asset_quarter = fund_asset[fund]
            fund_asset_quarter = fund_asset_quarter.loc[list(fund_holding.columns)]
            stock_ratio_quarter = stock_ratio[fund]
            stock_ratio_quarter = stock_ratio_quarter.loc[list(fund_holding.columns)]
            fund_stock_mv = fund_holding.mul(fund_asset_quarter, axis='columns') / 100.0
            fund_stock_mv = fund_stock_mv.mul(stock_ratio_quarter, axis='columns') / 100.0
    ...
    csv_file = r'C:\Users\doufucheng\OneDrive\Desktop\HolderBySFMVRatio.csv'
...
if __name__ == '__main__':
    ...
    data = HolderBySFMVRatio(beg_date, end_date)
