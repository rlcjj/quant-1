...
def PriceOpenAdjust(beg_date, end_date):
    ...
    # param
    #################################################################################
    factor_name = "PriceOpenAdjust"
    beg_date = Date().change_to_str(beg_date)
    end_date = Date().change_to_str(end_date)

    # read data
    #################################################################################
    price_unadjust = Stock().read_factor_h5("PriceOpenUnadjust")
...
if __name__ == "__main__":
    ...
    data = PriceOpenAdjust(beg_date, end_date)
    print(data)
