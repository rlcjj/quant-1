...
class InstitutionPassiveReport(Data):
...
if __name__ == '__main__':
    ...
    fund_code = "162216.OF"
    fund_name = "泰达宏利中证500指数"
    date = "20190131"
    fund_strategy = "配置盈利稳定增长、估值不高的公司，分散个股投资风险"
    asset_allocation_strategy = "战略上维持较高股票仓位，战术上根据宏观基本面、市场趋势、估值做适度调整"
    bench_code = '000905.SH'
    fund_manager = "刘洋"

    self = InstitutionPassiveReport()
    self.get_input_data(fund_code, fund_name, fund_strategy, asset_allocation_strategy, bench_code, date, fund_manager)
    # self.update_data()
    # self.write_excel()
    # self.write_word()
