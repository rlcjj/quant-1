...
def write_rs_gs(end_date, save_path):
    ...
    benchmark_code = "固定收益年化6.52%"
    benchmark_name = "固定收益年化6.52%"
    benchmark_code_2 = '中证500全收益指数30%'
    benchmark_name_2 = "中证500全收益指数30%"
    ...
    benchmark_array = np.array([["沪深300", "000300.SH"],
                                ["中证500", "000905.SH"],
                                ["中证500全收益", 'H00905.CSI'],
                                ["中证500全收益指数30%", '中证500全收益指数30%'],
                                ["固定收益年化6.52%", '固定收益年化6.52%']])
    ...
    chart_name = fund_name + "相对固定收益6.52%累计超额收益（成立以来）"
    insert_pos = 'B16'
    ...
    chart_name = fund_name + "相对500全收益指数30%累计超额收益（成立以来）"
    insert_pos = 'B32'
    excel.line_chart_time_series_plot(worksheet, 0, col_number, cum_return,
                                      series_name, chart_name, insert_pos, sheet_name)
    excel.close()
