...
class HoldingQuarter(Data):
    ...
    def get_data(self):
        ...
        self.free_mv = Stock().read_factor_h5("Mkt_freeshares", Stock().get_h5_path("mfc_primary")) / 100000000.0
    ...
    def cal_all_wind_file(self):
        ...
        for i_date in range(len(date_series)):
            ...
            free_mv_date = pd.DataFrame(self.free_mv[publish_date])
            free_mv_date.columns = ['FreeMV']

            data = pd.concat([stock_data_weight, free_mv_date], axis=1)
            data = data.dropna()
            ...
            for i in range(self.group):
                ...
                group_size = int(len(data) / group)
                bg_number = group_size * i
                ed_number = group_size * (i + 1)
                data_group = data.iloc[bg_number:ed_number, :]
                ...
                port_name = self.port_name + '_Size%s' % (i+1)
