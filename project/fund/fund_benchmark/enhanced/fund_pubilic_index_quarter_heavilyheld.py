...
class FundPublicIndexQuarterHeavilyHeld(object):
    ...
    def __init__(self):
        ...
        self.name = "PublicQuarterHeavilyHeld"
    ...
    def cal_quarter_holding_allfund_quarter(self, quarter_date):
        ...
        fund_weight = Fund().get_fund_top10_weight()
        fund_weight = fund_weight.loc[fund_pool, :]
        fund_weight_date = pd.DataFrame(fund_weight[quarter_date])
        fund_weight_date = fund_weight_date.dropna()
        fund_weight_date = fund_weight_date.sort_values(by=[quarter_date], ascending=False)
        fund_pool = list(fund_weight_date.index[0:int(len(fund_weight_date)/2)])
