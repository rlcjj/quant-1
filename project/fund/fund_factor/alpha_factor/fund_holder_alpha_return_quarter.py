...
class FundHolderReturnQuarterBackTest(object):
    ...
    def cal_fund_holder_return_quarter_backtest(self, fund, T, end_date, col="AlphaReturn", type="Mean"):

        beg_date = Date().get_trade_date_offset(end_date, -T)
        data = FundHolderRiskAlphaReturnQuarter().get_fund_holder_risk_alpha_return_quarter(fund, end_date)
    ...
    def cal_fund_holder_return_quarter_backtest_all(self, T, beg_date, end_date,
        ...
        for i in range(len(date_series)):
            ...
            for i_fund in range(len(fund_code_list)):
                ...
                try:
                    res = self.cal_fund_holder_return_quarter_backtest(fund, T, report_date, col, type)
                    result.loc[report_date, fund] = res
                except Exception as e:
                    result.loc[report_date, fund] = np.nan

        result = result.T
        file = os.path.join(self.path, "FundHolderQuarter_" + col + type + "_" + str(T) + '.csv')
        result.to_csv(file)
...
if __name__ == "__main__":
    ...
    # col = "AlphaReturn"
    # type = "Mean"
    # FundHolderReturnQuarterBackTest().cal_fund_holder_return_quarter_backtest_all(T, beg_date, end_date, col, type)
    # type = "IR"
    # FundHolderReturnQuarterBackTest().cal_fund_holder_return_quarter_backtest_all(T, beg_date, end_date, col, type)
